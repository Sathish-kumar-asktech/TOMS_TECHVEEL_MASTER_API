...
const Supplier = () => {
  ...
  const InsertData = async (newRecord) => {
    try {
      const res = await axios.instance.post("/InsertSupplier", newRecord, {
        headers: {
  ...
  const updateData = async (editID, updatedData) => {
    try {
      await axios.instance.put(`/UpdateSupplier/${editID}`, updatedData, {
        headers: {
          Authorization: tokent,
          "Content-Type": "application/json",
        },
  ...
  const deleteUser = async (deleteId) => {
    try {
      await axios.instance
        .delete(`/DeleteSupplier/${deleteId}`, {
          headers: {
            Authorization: tokent,
            "Content-Type": "application/json",
          },
  ...
  const handleInputChange = (event) => {
    ...
    if (name === "SupplierName") setSupplierName(value);
  ...
  const validateInput = (name, value) => {
    ...
    const isDataExists = allData.some(
      (data) => data.SupplierName.toLowerCase() === value.toLowerCase().trim()
    );
    const isDataExistsinEdit = allData.some(
      (data) =>
        data.SupplierName.toLowerCase() === value.toLowerCase().trim() &&
        data.Supplierid !== editID
    );
    ...
    if (name === "SupplierName") {
      if (!value.trim()) {
        error = true;
        helperText = "Supplier Name field cannot be empty";
        setIsFormSubmitted(false);
      } else if (operation === "Add" && isDataExists) {
        error = true;
        helperText = "Supplier Name already exists";
        setIsFormSubmitted(false);
          ...
          <Table>
            ...
            <TableBody>
              {(rowsPerPage > 0
                ...
                <TableRow key={data.Supplierid} hover>
            ...
            <form onSubmit={handleSubmit}>
              ...
              <Stack direction={"column"} spacing={2} p={2}>
                <TextField
                  ...
                  helperText={helperTexts.SupplierName}
                  className={
                    isFormSubmitted && errors.SupplierName
                      ? "shake-helper-text"
                      : ""
                  }
                />

...
const ProductCategory = () => {
  ...
  const handleEdit = (data) => {
    ...
    setEditID(data.ProductCategoryid);
    setProductCategory(data.ProductCategoryName);
  ...
  const deleteUser = async (deleteId) => {
    try {
      await axios.instance
        .delete(`/DeleteProductCategory/${deleteId}`, {
          headers: {
            Authorization: tokent,
            "Content-Type": "application/json",
          },
  ...
  const validateInput = (name, value) => {
    ...
    const isDataExists = allData.some(
      (data) =>
        data.ProductCategoryName.toLowerCase() === value.toLowerCase().trim()
    );
    const isDataExistsinEdit = allData.some(
      (data) =>
        data.ProductCategoryName.toLowerCase() === value.toLowerCase().trim() &&
        data.ProductCategoryid !== editID
    );

    if (name === "ProductCategory") {
      if (!value.trim()) {
        error = true;
        helperText = "Product Category field cannot be empty";
        setIsFormSubmitted(false);
      } else if (operation === "Add" && isDataExists) {
        error = true;
        helperText = "product category already exists";
        setIsFormSubmitted(false);
      ...
      if (!value.trim()) {
        error = true;
        helperText = "Plaese select active status";
        setIsFormSubmitted(false);
      } else {
        setIsFormSubmitted(true);
      }
          ...
          <Table>
            ...
            <TableBody>
              {(rowsPerPage > 0
                ...
                <TableRow key={data.ProductCategoryid} hover>
                  ...
                  <TableCell align="center" sx={{ padding: "0" }}>
                    ...
                    <IconButton
                      aria-label="Delete"
                      onClick={() => handleDelete(data.ProductCategoryid)}
                    >
            ...
            <form onSubmit={handleSubmit}>
              ...
              <Stack direction={"column"} spacing={2} p={2}>
                <TextField
                  ...
                  value={ProductCategory === null ? "" : ProductCategory}
                  onBlur={handleBlur}
                  onChange={handleInputChange}
                  fullWidth
                  size="small"
                  error={errors.ProductCategory}
                  helperText={helperTexts.ProductCategory}
                  className={

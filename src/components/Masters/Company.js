...
const Company = () => {
  ...
  const getData = async () => {
    setOpenLoader(true);
    try {
      const res = await axios.instance.get("/GetAllCompany", {
        headers: { Authorization: tokent, "Content-Type": "application/json" },
      });
      setallData(res.data);
      setOpenLoader(false);
  ...
  const InsertData = async (newRecord) => {
    try {
      const res = await axios.instance.post("/InsertCompany", newRecord, {
        headers: {
  ...
  const handleEdit = (data) => {
    ...
    const selectedBank = allbankData.find(
      (udata) => udata.Bankid === data.Bankid
    );
  ...
  const updateData = async (editID, updatedData) => {
    try {
      await axios.instance.put(`/UpdateCompany/${editID}`, updatedData, {
        headers: {
          Authorization: tokent,
          "Content-Type": "application/json",
        },
  ...
  const handleSubmit = (event) => {
    ...
    if (inputValidation) {
      ...
      if (!Bankid) {
        setErrors((prevErrors) => ({ ...prevErrors, Bankid: true }));
        setHelperTexts((prevHelperTexts) => ({
          ...prevHelperTexts,
  ...
  const handleInputChange = (event) => {
    ...
    if (name === "Bankid") setBankid(value);
  ...
  const handleInputChangeBank = (event, newValue) => {
    setBankid(newValue);
    ...
    if (!newValue) {
      ...
      setErrors((prevErrors) => ({ ...prevErrors, Bankid: false }));
      setHelperTexts((prevHelperTexts) => ({ ...prevHelperTexts, Bankid: "" }));
    }
  };
  ...
  const validateInput = (name, value) => {
    ...
    const isDataExists = allData.some(
      (data) => data.CompanyName.toLowerCase() === value.toLowerCase().trim()
    );
    const isDataExistsinEdit = allData.some(
      (data) =>
        data.CompanyName.toLowerCase() === value.toLowerCase().trim() &&
        data.CompanyId !== editID
    );
    ...
    if (name === "CompanyName") {
      if (!value.trim()) {
        error = true;
        helperText = "Company Name field cannot be empty";
        setIsFormSubmitted(false);
      } else if (operation === "Add" && isDataExists) {
        error = true;
        helperText = "Company Name already exists";
        setIsFormSubmitted(false);
          ...
          <Table>
            ...
            <TableBody>
              {(rowsPerPage > 0
                ...
                <TableRow key={data.CompanyId} hover>
            ...
            <form onSubmit={handleSubmit}>
              ...
              <Stack direction={"column"} spacing={2} p={2}>
                <TextField
                  ...
                  helperText={helperTexts.CompanyName}
                  className={
                    isFormSubmitted && errors.CompanyName
                      ? "shake-helper-text"
                      : ""
                  }
                />
              ...
              <Stack direction={{ md: "row", sm: "column" }} spacing={2} px={2}>
              ...
              <Stack direction={{ md: "row", sm: "column" }} spacing={2} p={2}>
              ...
              <Stack direction={"column"} spacing={2} p={2}>
                <FormControl size="small" fullWidth component="fieldset">
                  <Autocomplete
                    ...
                    options={allbankData}
                    getOptionLabel={(data) =>
                      data ? `${data.Bankname}- ${data.AccNum}` : ""
                    }
                    value={Bankid || null}
                  ...
                  {errors.Bankid && (
                    <Typography
                      ...
                      color="error"
                      className={
                        isFormSubmitted && errors.Bankid
                          ? "shake-helper-text"
                          : ""
                      }
                      sx={{ ml: 0 }}
                    >
                      {errors.Bankid && helperTexts.Bankid}
                    </Typography>
                  )}
                </FormControl>
                <FormControl size="small" fullWidth component="fieldset">

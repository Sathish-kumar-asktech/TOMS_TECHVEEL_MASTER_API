...
const ManagePurchaseInvoice = () => {
  ...
  const getAllPurchaseInvoice = async () => {
    setOpenLoader(true);
    try {
      const res = await axios.instance.get("/GetAllPurchaseInvoice", {
  ...
  const deleteInvoice = async (DataId) => {
    try {
      await axios.instance
        .delete(`/DeletePurchaseInvoice/${DataId}`, {
          headers: {
  ...
  const handleNewInvoice = () => {
    navigate("/transacation/PurchaseInvoice");
  };

  const EditPurchaseInvoice = (editID) => {
    navigate(`/transacation/PurchaseInvoice/${editID}`);
  };
          ...
          <Table>
            <TableHead>
              <TableRow sx={{ padding: "none" }}>
                ...
                <StyledTableCell>PI Number</StyledTableCell>
                <StyledTableCell>PI Date</StyledTableCell>
            ...
            <TableBody>
              {(rowsPerPage > 0
                ...
                <TableRow key={data.PurchaseInvoiceID} hover>
                  ...
                  <TableCell align="center">{data.PINumber}</TableCell>
                  <TableCell align="center">
